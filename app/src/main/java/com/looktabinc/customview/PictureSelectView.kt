...
import com.looktabinc.databinding.LayoutPictureBinding

class PictureSelectView @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null
) : LinearLayout(context, attrs) {

    private var binding: LayoutPictureBinding? = null
    val map = mapOf<Int, String>(
        1 to resources.getString(R.string.shop),
        2 to resources.getString(R.string.food),
        3 to resources.getString(R.string.selfy)
    )
    ...
    private fun initView() {
        binding = LayoutPictureBinding.inflate(LayoutInflater.from(context), this, false)
        setBasicStyle()
        clickListener()
    ...
    private fun clickListener() {
        binding?.layoutItemShop?.setOnClickListener {
            setValue(it, 1)
        }
        binding?.layoutItemFood?.setOnClickListener {
            setValue(it, 2)
        }
        binding?.layoutItemSelfy?.setOnClickListener {
            setValue(it, 3)
        }
    }
